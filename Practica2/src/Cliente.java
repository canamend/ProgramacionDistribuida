...
public class Cliente {
   ...
   static private boolean conectarServidor(int maxIntentos, String ipServer, int serverPort ){
		...
		while((van<maxIntentos) && !exito){
			try {
				socketAlServidor = new Socket(ipServer, serverPort);
				exito = true;
			} catch (Exception e) {
				van++;
	...
	public static void main(String[] args) {
		...
		exito = conectarServidor(10, args[0], Integer.parseInt(args[1])); //10 intentos
